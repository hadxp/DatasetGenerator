...
from utils import ResultEntry, get_image_files, get_video_files, save_images
...
def setup_argparse() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--qwen', action='store_true', default=False, help='Force uses qwen to describe images or videos')
    parser.add_argument('--no_check', action='store_true', default=False, help='Skips gen.txt check')
        ...
        if regernerate_dataset:
            ...
            for i, file_path in enumerate(tqdm(files), 1):
                ...
                if is_video:
                    ...
                    for frame in video.decode(video=0):
                        ...
                        img = preprocess_image(frame_img, upscale_processor, upscale_model, imgwidth, imgheight)
                        frames.append(img)
                elif is_image:
                ...
                caption = generate_caption(
                    model=model,
                    processor=processor,
                    source_object=frames if is_video else img,
                    task=task,
            ...
            if jsonl and write_task_handled is False:
                ...
                if results:
                    ...
                    save_image_entries = save_images(results, target_dir)
                    # create jsonl
                    jsonl_path = target_dir / "0_dataset.jsonl"
                    print(f"Writing {len(results)} results to JSONL: {jsonl_path}")
                    with open(jsonl_path, 'w', encoding='utf-8') as f:
                        dump = '\n'.join(json.dumps(save_image_entry, ensure_ascii=False) for save_image_entry in save_image_entries)
                        f.write(dump)
            ...
            if not write_task_handled:
                if results:
                    ...
                    save_image_entries = save_images(results, target_dir)
                    # create text files
                    print(f"Creating text (caption) files from {len(results)} results...")
                    for save_image_entry in save_image_entries:
                        # get the filepath of the image, without its extension
                        text_filename = Path(save_image_entry["image_path"]).stem
                        text_filepath = os.path.join(target_dir, text_filename)
                        Path(text_filepath).write_text(save_image_entry["caption"], encoding='utf-8')
